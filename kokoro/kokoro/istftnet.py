...
class Generator(nn.Module):
    def __init__(
        ...
        if adapters is not None and len(adapters) != self.num_upsamples:
            raise ValueError(
                f"Generator adapters length {len(adapters)} != num_upsamples {self.num_upsamples}"
            )
    ...
    def forward(self, x, s, f0, z_style: Optional[torch.Tensor] = None):
        ...
        x = F.leaky_relu(x)
        x = self.conv_post(x)
        spec = torch.exp(x[:,:self.post_n_fft // 2 + 1, :].clamp(max=15.0))
...
# Channel counts at each optional L-adapter site: after encode, then after each decode block.
DECODER_L_ADAPTER_HIDDEN_DIMS = (1024, 1024, 1024, 1024, 512)


def generator_l_adapter_hidden_dims(upsample_initial_channel: int, num_upsamples: int) -> tuple[int, ...]:
    """Channel width at each optional Generator L-adapter site (after each upsample/resblock stack)."""
...
class Decoder(nn.Module):
    def __init__(self, dim_in, style_dim, dim_out, 
                 ...
                 disable_complex=False,
                 decoder_adapters: Optional[nn.ModuleList] = None,
                 generator_adapters: Optional[nn.ModuleList] = None):
        super().__init__()
        self.decoder_adapters = decoder_adapters
        if decoder_adapters is not None:
            if len(decoder_adapters) != len(DECODER_L_ADAPTER_HIDDEN_DIMS):
                raise ValueError(
                    f"decoder_adapters length {len(decoder_adapters)} must be "
                    f"{len(DECODER_L_ADAPTER_HIDDEN_DIMS)} (encode + each decode block)"
                )
